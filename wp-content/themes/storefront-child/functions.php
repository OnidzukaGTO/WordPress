<?php
// Скрытие опций оплаты и установка метода оплаты по умолчанию
add_action('after_setup_theme', 'customize_checkout_payment_options');

function customize_checkout_payment_options() {
    // Скрыть опции оплаты с помощью CSS и JavaScript
    add_action('wp_head', 'hide_checkout_payment_options');

    // Установить метод оплаты по умолчанию
    add_filter('woocommerce_available_payment_gateways', 'set_default_payment_gateway');
}

function hide_checkout_payment_options() {
    if (is_checkout()) {
        ?>
        <style>
...
function set_default_payment_gateway($available_gateways) {
    if (!is_admin()) {
        // Проверяем, что доступен хотя бы один метод оплаты
...
function get_element_name_by_number($number) {
    $elements = [
...
add_filter('woocommerce_email_subject_customer_processing_order', 'custom_email_subject', 10, 2);
...
function custom_email_subject($subject, $order) {
    ...
    // Получаем название химического элемента по количеству товаров
    $element_name = get_element_name_by_number($total_quantity);

    return $element_name ? $element_name : $subject;
}
